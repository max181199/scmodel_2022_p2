...
void fill(Point* p, long len, int size) {
  ...
  for (int i = 0; i < len; i++){
    if (diff > 0){
      ...
      p[i].x = 2 * ((double)(rand()) / RAND_MAX) - 1;
      p[i].y = 2 * ((double)(rand()) / RAND_MAX) - 1;
      p[i].z = 4 * ((double)(rand()) / RAND_MAX) - 2;
      //printf("X,");
    }
  }
  //printf("\n");
...
double F(Point * p) {
  if ( ((fabs(p -> x) + fabs(p -> y)) <= 1) && (-2 <= p -> z) && (p -> z <= 2))
...
int main(int argc, char* argv[]) {
  ...
  if (rank == 0){
    ...
    while (fabs(16.0 / 135 - integral) > epsilon) {
    ...
    printf("RES=%.10lf, DEF=%.10lf,  NUM=%d;\n", integral, fabs(I - integral), (scale - 1) * N );
